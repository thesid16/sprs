...
Authoritative dataset: /home/rohit/tournament_p1/results/  (July clean Phase-1
...
# Round-3 fix: this line used to hardcode live_hw_results_p1_unified.csv, which
# hwdata documents as DEFECTIVE for any cycle-derived claim. Every other
# generator (gen_tournament.py, gen_cost_tables.py, gen_coverage.py) resolves
# its source through hwdata.select(); this one did not, so `make tables` emitted
# a per-instance table with 14 schedules beating the paper's own proven lower
# bound. The tables it writes are not \input by main.tex, so no built PDF was
# affected -- but the generator ships in the artifact, and a referee who runs it
# gets the contradiction. Resolve the source the same way as everyone else.
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import hwdata

_HW_ROWS, HW_CSV, _HW_TRUSTED = hwdata.select(quiet=True)
MAXP_CSV = os.environ.get("SPRS_MAXP_CSV") or os.path.join(P1, "live_hw_results_maxp_mod_fat.csv")
...
def canonical_roots():
    """Expected 64-bit root per N.

    Recomputed from the shipped compiler (src/sprs_core.py) rather than by
    scanning the 326 GB emitted testbench corpus, which the release does not
    ship. EXPECTED is a function of (N, alu_op) alone, so this is exact.
    The compiled (label,G,topology) set is taken from the released
    per-instance sw_results.csv, one row per feasible pipeline.
    """
    ...
    for lab, (N, G, T) in META.items():
        swp = f"{P1}/{lab}/sw_results.csv"
        if not os.path.exists(swp):
            continue
        has = False
        with open(swp, newline='', encoding='utf-8', errors='replace') as f:
            for r in csv.DictReader(f):
                if (r.get('tb_fname') or '').strip():
                    has = True
                    break
        if not has:
            continue
        if N not in rootcache:
            tree = sc.CBTree(N); topo = sc.build_topology('linear', 2)
            _, s2f = sc.STAGE2_METHODS['2b']; _, s3f = sc.STAGE3_METHODS['3a']
            a = s2f(tree, 2, topo, sc.HW); m = s3f(tree, a, 2, topo, sc.HW)
            sch = sc.build_schedule(tree, a, m, 2, topo, sc.HW, alu_op=sc.ALU_FADD)
            pr = sc.generate_instructions(tree, topo, sch, sc.HW, alu_op=sc.ALU_FADD)
            tb = sc.emit_sv_testbench(tree, topo, pr, assignment=a,
                                      addr_alloc=sch.addr_alloc, schedule=sch,
                                      alu_op=sc.ALU_FADD)
            rootcache[N] = re.search(r"EXPECTED\s*=\s*64'h([0-9A-Fa-f]{16})", tb).group(1).upper()
        byN[N][rootcache[N]].add((lab, G, T))
    return byN
...
def write(out_dir, name, body, source, note=""):
    # If the trusted max-over-PEs re-measurement is unavailable, hwdata falls
    # back to the defective source. Stamp that in the output so the `make check`
    # guard (which greps tables/ for DEFECTIVE) can actually see it -- previously
    # this script wrote no stamp at all, so the guard was blind to it.
    if not _HW_TRUSTED:
        note = (note + " " if note else "") + "DEFECTIVE: cycle-derived numbers below come from the GPU-0 metric."
...
def tbl_invariance(out_dir, hw, roots):
    ...
    mut = {}
    # Resolve relative to this script, not to --out: with --out pointing anywhere
    # other than paper/tables the mutation JSON was silently not found and the
    # two mutation columns degraded to \todo{--} without any diagnostic.
    _paper = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    mpath = os.path.join(_paper, "data", "mutation_results.json")
    ...
    note = (f"{tot} passing HW configs; every N has exactly 1 distinct root; "
            f"root column recomputed from src/sprs_core.py (EXPECTED is a function "
            f"of N and alu_op alone), not scanned from the unreleased TB corpus; "
...
def main():
    ...
    print(f"loading authoritative dataset from {P1} ...")
    ...
    print("recomputing canonical EXPECTED roots from src/sprs_core.py ...")
