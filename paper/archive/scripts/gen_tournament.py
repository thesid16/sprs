...
def main():
    ...
    # ---- per-instance ----
    # The assignment ($\asgn$) and mapping ($\map$) columns are NOT emitted.
    # Council T15: the per-instance "winner" is a CSV-arrival-order tie-break
    # artifact on 11 of 39 rows for the assignment and 38 of 39 for the mapping,
    # so naming one algorithm per row asserts a discrimination the data does not
    # support. They were deleted from the manuscript's copy of this table by
    # hand on 2026-08-26; the generator kept re-creating them, so `make tables`
    # silently reinstated a refuted claim. Removed at the source instead.
    T15_NOTE = ("assignment/mapping columns dropped -- champion attribution is a\n"
                "%          tie-break artifact on 11 and 38 of 39 rows respectively (T15).")
    b = [r"\begin{tabular}{@{}rrlrr@{}}", r"\toprule",
         r"$N$ & $G$ & Topology & cycles & cyc/$\Omega$ \\",
         r"\midrule"]
    for n, g, t, s2, s3, cyc, o in sorted(per):
        r = f"{cyc/o:.2f}" if o else "--"
        b.append(f"{n} & {g} & {TOPO[t]} & {cyc} & {r} \\\\")
    b += [r"\bottomrule", r"\end{tabular}"]
    open(f"{a.out}/perinstance_full.tex", "w").write(
        hdr(src, trusted, T15_NOTE) + "\n".join(b) + "\n")
