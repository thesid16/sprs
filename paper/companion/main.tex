...
The \sprs{} fabric is $3{,}738$ lines of SystemVerilog across thirteen
...
\sv{noc\_router.sv}         & $N$-port multi-VC router      & 301 \\
\sv{noc\_ni.sv}             & network interface, GCI buffer & 217 \\
\sv{fp64\_add.sv}           & IEEE-754 FP64 adder           & 242 \\
\sv{btree\_fsm\_fast.sv}    & per-PE reduction pipeline     & 918 \\
...
\midrule
\multicolumn{2}{@{}l}{\textbf{Total}} & \textbf{3{,}738} \\
\bottomrule
\end{tabular}
\end{table}

% =====================================================================
\section{Instruction and Packet Encodings}\label{sec:packet}
% =====================================================================
...
[59:56] aux      aux[0]=legacy link hint, aux[3:1]=alu_op
...
Egress port selection is not carried in the instruction: one bit could not
address a router's egress ports, of which there are up to 66 in this suite
(\S\ref{sec:router}). The port is resolved by the
router's $\sv{route\_table}[\sv{dest\_gpu}]$ lookup, with \sv{dest\_gpu} taken
from \sv{addr\_b} on a \texttt{SEND}. \sv{aux}[0] is a vestigial hint retained
for encoding compatibility and ignored by the router.
...
96-bit datapath width. Virtual-channel selection reads bit~64 --- the
low bit of the reserved header field, immediately \emph{above} the 64-bit
payload --- so VC choice is independent of the transported value. This
matters: bit~63 is the sign bit of an FP64 payload, and steering on it would
make buffer occupancy and contention depend on whether a partial sum happens
to be negative. The RTL comparison is
\sv{rx\_data[p][64 -: VC\_W]} in \sv{noc\_router.sv}. At the default
\sv{NUM\_VCS} $=2$ ($\sv{VC\_W}=1$) this reads bit~64 alone; note that at
\sv{NUM\_VCS} $=4$ the same expression would extend down into payload bit~63,
so raising the VC count requires moving the field rather than widening it in
place.
...
$\lceil\log_2(\text{NUM\_PORTS})\rceil$ bits wide, and \sv{NUM\_PORTS} is
\sv{PORTS\_PER\_RTR}, a per-instance value the compiler emits --- not the
constant \sv{MAX\_PORTS}, which is declared in \sv{noc\_pkg.sv} and
referenced by no module. Mesh and torus use five-port routers, giving three
bits; hypercube reaches eleven ports at $G = 1024$ (four bits); fat-tree
reaches \emph{sixty-six} at $G = 4096$, needing seven. Any tooling that
writes the table with a sized literal (\sv{3'd}$\langle p\rangle$, e.g.\
\sv{3'd7}) silently truncates every port number above seven and breaks
fat-tree and large-hypercube routing in a way that passes elaboration and
fails only at run time. Plain integer literals, which widen to the
left-hand side, are required.
...
% =====================================================================
\section{Router}\label{sec:vc}\label{sec:router}
% =====================================================================
...
selected by packet bit~64 (\S\ref{sec:packet}), that is, by the compiler
rather than by hardware policy. This keeps the router simple and puts deadlock avoidance where the
